...
const TAB_HELP={
  ...
  coach:{t:"AI Coach",b:`<div class="help-body">
    ...
    <div class="help-row"><b>BUILD A SESSION</b><span>Choose the muscle groups and length you want — or tap “Recommend one for me” and the Coach picks based on your goal and recent training. Then Start it now or SAVE FOR LATER.</span></div>
    <div class="help-row"><b>ANALYSE LOGS</b><span>Reviews your recent workouts &amp; targets and suggests improvements.</span></div>
...
function renderHome(){
  ...
  else { heroTitle="GET TO WORK."; heroSub="Pick a muscle. Start a session."; heroBtn="⚡ Quick start"; heroAct=openQuickStart; }
...
function renderTrain(){
  ...
  if(DATA.favWorkouts&&DATA.favWorkouts.length){
    ...
    empty.innerHTML=`<div class="saved-empty">
      ...
      <div class="saved-empty-s">Save any session with ★. The Coach can also build one.</div>
    </div>`;
    b.appendChild(empty);
  }
}
function cardioEntryCard(){
  ...
  cc.innerHTML=`<div class="mega-glow" style="background:radial-gradient(120% 120% at 0% 0%,rgba(47,230,168,.3),transparent 50%),radial-gradient(120% 120% at 100% 100%,rgba(90,169,255,.22),transparent 50%)"></div>
    ...
    <div class="ct">${CARDIO.length} activities · timers & calories</div></div>`;
  cc.addEventListener("click",()=>switchTab("cardio"));
  return cc;
}
function renderGymZone(b){
  /* ── TARGET: single column of muscle group rows ── */
  const hdr=el("div","b-sec-hdr"); hdr.innerHTML=`<span class="b-sec-lbl">SELECT MUSCLE GROUP</span>`; b.appendChild(hdr);
  const groupList=el("div","b-group-list");
  GROUPS.forEach(g=>{
    const row=el("button","b-group-row");
    const n=gymExercisesIn(g).length;
    const gColor=GROUP_GLOW[g]||"var(--line)";
    row.innerHTML=`<div class="bgr-dot" style="background:${gColor}"></div>
      <div class="bgr-left">
        <div class="bgr-name"><span class="bgr-icon">${GICON[g]||""}</span>${g.toUpperCase()}</div>
        <div class="bgr-count">${n} EXERCISES</div>
      </div><div class="bgr-arrow">→</div>`;
    row.addEventListener("click",()=>openGroupBuilder(g));
    groupList.appendChild(row);
  });
  b.appendChild(groupList);

  /* ── QUICK ACTIONS ── */
  const qhdr=el("div","b-sec-hdr"); qhdr.innerHTML=`<span class="b-sec-lbl">QUICK START</span>`; b.appendChild(qhdr);
  const actList=el("div","b-action-list");

  const megaRow=el("button","b-action-row");
  megaRow.innerHTML=`<div class="bar-left"><div class="bar-name">MEGA SESSION</div><div class="bar-sub">Multiple muscle groups + cardio</div></div><div class="bar-arrow">→</div>`;
  megaRow.addEventListener("click",openMegaBuilder); actList.appendChild(megaRow);

  const cardioRow=el("button","b-action-row");
  cardioRow.innerHTML=`<div class="bar-left"><div class="bar-name">CARDIO</div><div class="bar-sub">${CARDIO.length} activities — timers & calorie tracking</div></div><div class="bar-arrow">→</div>`;
  cardioRow.addEventListener("click",()=>switchTab("cardio")); actList.appendChild(cardioRow);
  b.appendChild(actList);

  /* ── PRESET SESSIONS ── */
  const phdr=el("div","b-sec-hdr"); phdr.innerHTML=`<span class="b-sec-lbl">PRESET SESSIONS</span><span class="b-sec-sub">TAP TO START</span>`; b.appendChild(phdr);
  const presetList=el("div","b-action-list");
  const DAY_DESC={Push:"CHEST · SHOULDERS · TRICEPS",Pull:"BACK · BICEPS · REAR DELTS",Legs:"QUADS · HAMSTRINGS · GLUTES","Full Body":"EVERY MUSCLE GROUP",Upper:"CHEST · BACK · SHOULDERS · ARMS",Lower:"LEGS · GLUTES · CORE"};
  Object.keys(PRESET_DAYS).forEach(d=>{
    const list=PRESET_DAYS[d];
    const row=el("button","b-action-row");
    row.innerHTML=`<div class="bar-left"><div class="bar-name">${d.toUpperCase()}</div><div class="bar-sub">${DAY_DESC[d]||list.length+" exercises"}</div></div><div class="bar-count-badge">${list.length}</div>`;
    row.addEventListener("click",()=>startSession(d,"preset",PRESET_DAYS[d].map(n=>mkExercise(n))));
    presetList.appendChild(row);
  });
  b.appendChild(presetList);
}
function renderHomeZone(b){
  const hdr=el("div","b-sec-hdr"); hdr.innerHTML=`<span class="b-sec-lbl">HOME TRAINING</span>`; b.appendChild(hdr);
  const list=el("div","b-action-list");

  const buildRow=el("button","b-action-row");
  buildRow.innerHTML=`<div class="bar-left"><div class="bar-name">BUILD A SESSION</div><div class="bar-sub">Choose your equipment — we pick the best moves</div></div><div class="bar-arrow">→</div>`;
  buildRow.addEventListener("click",openHomeBuilder); list.appendChild(buildRow);

  const bwRow=el("button","b-action-row");
  bwRow.innerHTML=`<div class="bar-left"><div class="bar-name">BODYWEIGHT CIRCUIT</div><div class="bar-sub">6 moves, no equipment, built in seconds</div></div><div class="bar-arrow">→</div>`;
  bwRow.addEventListener("click",()=>{
    ...
    startSession("BODYWEIGHT CIRCUIT","home",ex);
  }); list.appendChild(bwRow);

  const megaRow=el("button","b-action-row");
  megaRow.innerHTML=`<div class="bar-left"><div class="bar-name">HOME MEGA</div><div class="bar-sub">Full body + cardio finisher</div></div><div class="bar-arrow">→</div>`;
  megaRow.addEventListener("click",()=>{DATA.prefs.env="home";openMegaBuilder();}); list.appendChild(megaRow);

  const cardioRow=el("button","b-action-row");
  cardioRow.innerHTML=`<div class="bar-left"><div class="bar-name">CARDIO</div><div class="bar-sub">${CARDIO.length} activities</div></div><div class="bar-arrow">→</div>`;
  cardioRow.addEventListener("click",()=>switchTab("cardio")); list.appendChild(cardioRow);
...
function openQuickStart(){
  ...
  openModal(`<h3>Start a workout</h3>
   ...
   <div class="eyebrow" style="margin-bottom:8px">TARGET A MUSCLE</div>
   <div class="row wrap" style="gap:8px">
     ${GROUPS.map(g=>`<button class="chip" data-g="${g}">${g}</button>`).join("")}
   </div>
   <div class="divider"></div>
   <div class="eyebrow" style="margin-bottom:8px">PRESET SESSIONS</div>
   <div class="row wrap" style="gap:8px">
...
function openMachinePicker(group){
  ...
  openModal(`<h3>ADD EXERCISE</h3>
    <input class="input b-search-input" id="mp_search" placeholder="SEARCH..." style="margin:8px 0 10px">
    <div class="b-filter-scroll" style="margin-bottom:12px">
      ${GROUPS.map(g=>`<button class="b-filter-btn ${g===group?"on":""}\" data-g="${g}">${g.toUpperCase()}</button>`).join("")}
      <button class="b-filter-btn" data-g="FAV">★ STARRED</button>
      ${showFW?`<button class="b-filter-btn" data-g="FW">FREE WEIGHTS</button>`:""}
      ${inLive?`<button class="b-filter-btn" data-g="CARDIO">CARDIO</button>`:""}
    </div>
    <div class="b-ex-list" id="mp_list"></div>`);
  let curG=group;
  function addStrength(n){
    ...
    else if(liveSession){ liveSession.exercises.push(mkExercise(n)); renderLive(); toast("ADDED."); }
  }
  function paint(){
    const q=$("#mp_search").value.toLowerCase();
    if(curG==="CARDIO"){
      const list=CARDIO.filter(c=>c.n.toLowerCase().includes(q));
      $("#mp_list").innerHTML=list.map(c=>`<div class="b-ex-row">
        <div class="b-ex-info"><div class="b-ex-name">${esc(c.n).toUpperCase()}</div><div class="b-ex-group">${c.t==="machine"?"MACHINE CARDIO":"HOME CARDIO"}</div></div>
        <button class="b-ex-add" data-cardio="${esc(c.n)}">+</button></div>`).join("")||`<div class="empty">NO MATCHES</div>`;
      $("#mp_list").querySelectorAll("[data-cardio]").forEach(btn=>btn.addEventListener("click",()=>{
        ...
        if(liveSession){ liveSession.exercises.push(mkCardioCard(c)); renderLive(); toast("ADDED."); }
      }));
      return;
    }
    let pool;
    ...
    $("#mp_list").innerHTML=list.map(m=>`<div class="b-ex-row" data-n="${esc(m.n)}">
      <div class="b-ex-info">
        <div class="b-ex-name">${esc(m.n).toUpperCase()}</div>
        <div class="b-ex-group">${m.g.toUpperCase()}${m.fw?" · FREE WEIGHT":""}</div>
      </div>
      <div class="b-ex-btns">
        <button class="b-ex-guide" data-info="${esc(m.n)}">?</button>
        <button class="b-ex-star ${isFavMachine(m.n)?"on":""}" data-fav="${esc(m.n)}">${isFavMachine(m.n)?"★":"☆"}</button>
        <button class="b-ex-add" data-add="${esc(m.n)}">+</button>
      </div></div>`).join("")||`<div class="empty">${curG==="FAV"?"NOTHING STARRED YET.":"NO MATCHES"}</div>`;
...
function showHowTo(name){
  ...
  openModal(`<h3>${esc(m.n)}</h3><div class="eyebrow">${m.g} · ${m.fw?"free weights":"how to use"}</div>
    ...
    <div class="howto-block move"><div class="lab">MOVEMENT</div><p>${esc(h.move)}</p></div>
...
function maybeResumeWorkout(){
  ...
  openModal(`<h3>Resume your workout?</h3>
    ...
    <button class="btn block" id="rw_discard" style="margin-top:10px">START FRESH</button>`);
  $("#rw_resume").addEventListener("click",()=>{ closeModal(); resumeLive(saved); });
  $("#rw_discard").addEventListener("click",()=>{ clearLive(); closeModal(); toast("SESSION DISCARDED."); });
}
...
/* ── Session clock ── ticks every second in the live header ── */
let _clockTick=null;
function startSessionClock(){
  stopSessionClock();
  _clockTick=setInterval(()=>{
    const el=$("#live-clock"); if(!el||!liveSession)return;
    const secs=Math.floor((Date.now()-liveSession.startedAt)/1000);
...
function sessionClockStr(){
  if(!liveSession||!liveSession.startedAt) return "";
  const secs=Math.floor((Date.now()-liveSession.startedAt)/1000);
  const m=Math.floor(secs/60), s=secs%60;
...
function swapExercise(xi){
  ...
  $("#modal").querySelectorAll("[data-sw]").forEach(btn=>btn.addEventListener("click",()=>{
    ...
    ex.name=n; ex.group=m?m.g:g; delete ex.home; closeModal(); renderLive(); toast("SWAPPED — "+n);
  }));
}
...
function renderLive(){
  ...
  prog.innerHTML=`<div class="lp-row">
      <span class="lp-label" id="lp_exname"></span>
      <span class="lp-right"><span class="lp-clock" id="live-clock">${sessionClockStr()}</span><span class="lp-count" id="lp_num"></span></span>
    </div>
  ...
  liveSession.exercises.forEach((ex,xi)=>{
    ...
    /* ordinal */
    const ord=el("div","b-ex-ordinal");
    ord.textContent=`MOVE ${strengthI} OF ${strengthN}${ex.group?" · "+ex.group.toUpperCase():""}`;
    c.appendChild(ord);

    /* header: name | note btn | ⋯ */
    const hdr=el("div","b-ex-hdr");
    const nm=el("div","b-ex-title");
    nm.textContent=ex.name.toUpperCase();
    nm.addEventListener("click",()=>openExerciseHistory(ex.name));
    if(ss){const b=el("span","ss-badge"); b.textContent="SS"; nm.appendChild(b);}
    hdr.appendChild(nm);

    const acts=el("div","b-ex-hdr-actions");
    const nb=el("button","b-note-btn"+(liveSession.notes?" has-notes":""));
    nb.textContent="📝"; nb.title="Session notes";
    nb.addEventListener("click",openSessionNotes);
    acts.appendChild(nb);
    const more=el("button","b-more-btn","⋯");
    ...
    if(last&&(+last.kg>0||+last.reps>0)){
      const gh=el("div","b-active-ghost");
      gh.textContent=`👻 LAST SESSION: ${last.kg?liftStr(+last.kg):"—"} × ${last.reps||"—"}`;
      c.appendChild(gh);
    }
    ...
    /* add set */
    const addSetBtn=el("button","b-add-btn","＋ ADD SET");
    ...
    c.appendChild(addSetBtn);

    /* superset link */
    if(nx&&!nx.cardio){
      const foot=el("div","ss-foot");
      const lb=el("button","",linkedNext?"UNLINK SUPERSET":"＋ SUPERSET WITH NEXT");
    ...
    if(linkedNext){
      ...
      conn.innerHTML=`<span class="ln"></span><span>SUPERSET — NO REST</span><span class="ln"></span>`;
      body.appendChild(conn);
    }
  });

  const addEx=el("button","btn block add-ex","＋ ADD EXERCISE");
  addEx.style.marginTop="16px";
...
function buildSetList(ex,xi){
  ...
  function paint(){
    ...
    ex.sets.forEach((st,si)=>{
      ...
      /* ── DONE ── single line, tap to undo ── */
      if(st.done){
        const r=el("div","b-set-done");
        const kg=(+st.kg>0)?liftStr(+st.kg):"—";
        const reps=st.reps||"—";
        const rir=(st.rir&&!st.warmup)?` · RIR ${st.rir==="F"?"FAIL":st.rir}`:"";
        const prBadge=st._newPR?`<span class="b-pr-badge">PR</span>`:"";
        r.innerHTML=`<span class="b-set-done-num">${st.warmup?"W":si+1}</span>
          <span class="b-set-done-val">${kg} × ${reps}${rir}</span>
          ${prBadge}
          <span class="b-set-done-chk">✓</span>`;
        r.addEventListener("click",()=>{ st.done=false; ex._active=si; paint(); updateLiveProg(); persistLive(); });
        wrap.appendChild(r);

      /* ── ACTIVE set ── */
      } else if(si===act){
        const r=el("div","b-active-set");

        /* label */
        const lbl=el("div","b-active-label");
        lbl.innerHTML=`<span class="b-active-num">${st.warmup?"WARM-UP":"SET "+(si+1)}</span><span class="b-active-badge">NOW</span>`;
        r.appendChild(lbl);
        ...
        const stepRow=el("div","b-active-steps");
        ...
        /* RIR collapsed */
        if(!st.warmup){
          const rirWrap=el("div");
          const rirToggle=el("button","b-rir-toggle"+(st.rir?" active":""),
            st.rir?`EFFORT — RIR ${st.rir==="F"?"FAIL":st.rir+" LEFT"}`:"EFFORT (OPTIONAL) ›");
          rirWrap.appendChild(rirToggle);
          const rirRow=el("div","b-rir-chips"+(st.rir?" open":""));
          [[" ? ","info"],["0","0"],["1","1"],["2","2"],["3+","3+"],["FAIL","F"]].forEach(([lbl,v])=>{
            ...
            chip.addEventListener("click",()=>{
              ...
              rirToggle.textContent=st.rir?`EFFORT — RIR ${st.rir==="F"?"FAIL":st.rir+" LEFT"}`:"EFFORT (OPTIONAL) ›";
        ...
        /* LOG SET — the big one */
        const comp=el("button","b-log-btn",`✓  LOG SET ${si+1}`);
        comp.addEventListener("click",()=>{
          ...
          st.done=true;
          /* ── PR CHECK ── */
          const kgLogged=+st.kg||0;
          if(kgLogged>0&&!st.warmup){
            const prevPR=DATA.ach.prs[ex.name]||0;
            if(kgLogged>prevPR){
              st._newPR=true;
              /* don't save the PR yet — saveWorkout does that — just flash */
              setTimeout(()=>{
                const msg=prevPR>0
                  ? `🏅 NEW PR — ${liftStr(kgLogged)} on ${ex.name}! (was ${liftStr(prevPR)})`
                  : `🏅 FIRST LOG — ${liftStr(kgLogged)} on ${ex.name} saved as PR`;
                toast(msg);
      ...
      /* ── TODO ── dimmed ── */
        ...
        const r=el("div","b-set-todo");
        const kg=(+st.kg>0)?liftStr(+st.kg):"—";
        const tgt=(st.reps&&+st.reps>0)?st.reps:(last&&last.reps?last.reps:"—");
        r.innerHTML=`<span class="b-set-todo-num">${st.warmup?"W":si+1}</span><span class="b-set-todo-val">${kg} × ${tgt}</span><span class="b-set-todo-chk"></span>`;
...
function openExerciseMenu(ex,xi){
  ...
  openModal(`<h3 style="margin-bottom:14px">${esc(ex.name)}</h3>
    <div class="menu-list">
      <button class="menu-item" data-a="fav">${isFav(ex.name)?"★ UNSTAR":"☆ STAR"}</button>
      <button class="menu-item" data-a="info">FORM GUIDE</button>
      ${!ex.home?`<button class="menu-item" data-a="swap">SWAP MOVE</button>`:""}
      ...
      ${isFW?`<button class="menu-item" data-a="plates">PLATES</button>`:""}
      ...
      <button class="menu-item danger" data-a="remove">× REMOVE</button>
    </div>`);
  const act=(a)=>{
...
function removeLiveExercise(xi){
  ...
  confirmModal({title:"REMOVE THIS MOVE?",danger:true,confirmText:"Remove",
...
function buildSetBlock(ex,st,si){
  ...
  function paint(){
    ...
    if(st.done){
      ...
      const btn=el("button","sb-complete done","✓  DONE — UNDO");
...
function showFinishSummary(wk,prs,badges){
  ...
  openModal(`
    <div class="sum-hero">
      <div class="sum-check">✓</div>
      <div class="sum-title">SESSION DONE.</div>
      <div class="sum-sub">${esc(wk.title)}${wk.durationMin?` · ${wk.durationMin} MIN`:""}</div>
    </div>

    <div class="sum-stats">
      <div class="sum-stat">
        <div class="sum-stat-v">${volStr(wk.volume)}</div>
        <div class="sum-stat-k">TOTAL LOAD</div>
      </div>
      <div class="sum-stat">
        <div class="sum-stat-v">${wk.durationMin||"—"}</div>
        <div class="sum-stat-k">MINUTES</div>
      </div>
      <div class="sum-stat">
        <div class="sum-stat-v">${totalSets}</div>
        <div class="sum-stat-k">SETS LOGGED</div>
      </div>
      <div class="sum-stat">
        <div class="sum-stat-v">${prs.length||"—"}</div>
        <div class="sum-stat-k">NEW PRs</div>
      </div>
    </div>

    ${showAch&&prs.length?`<div class="sum-pr-row">
      ${prs.map(p=>`<div class="sum-pr-item">🏅 PR — ${esc(p.name).toUpperCase()} ${liftStr(p.kg)}${p.prev?` <span class="sum-pr-prev">(was ${liftStr(p.prev)})</span>`:""}</div>`).join("")}
    </div>`:""}

    <div class="sum-streak">
      ${dispWorkoutStreak()} DAY STREAK · ${dispWorkouts()} SESSIONS TOTAL
    </div>
    ...
    <div class="sum-actions">
      <button class="btn str block" id="fs_repeat">↺ REPEAT THIS SESSION</button>
      <button class="btn block" id="fs_share" style="margin-top:1px">📸 SHARE CARD</button>
      <button class="btn ghost block" id="fs_close" style="margin-top:1px">DONE</button>
    </div>`);

  /* repeat: rebuild exercises from wk, stripping done flags */
  $("#fs_repeat").addEventListener("click",()=>{
    const exs=wk.exercises.map(e=>({
      name:e.name, group:e.group,
      sets:e.sets.map(s=>({kg:s.kg||"",reps:s.reps||"",done:false,warmup:!!s.warmup,rir:null}))
    }));
...
function renderCardioSession(b){
  ...
  $("#cd_cancel").addEventListener("click",()=>{
    confirmModal({title:"Stop this cardio?",danger:true,confirmText:"START FRESH",body:"Are you sure? This session won't be logged.",
...
function renderStats(){
  ...
  function buildMega(body){
    ...
    if(!megas.length){ body.appendChild(el("div","empty","No Mega workouts yet — build one on the Train tab (look for MEGA SESSION). It mixes several muscle groups in one session.")); return; }
